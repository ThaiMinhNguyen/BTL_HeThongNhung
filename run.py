...
class DrowningDetectionApp:
    def __init__(self, root):
        ...
        # Các biến xử lý video
        self.cap = None
        self.is_playing = False
        self.thread = None
        self.current_frame = None
        
        # Các biến tối ưu hiệu suất
        self.target_fps = 15  # FPS mục tiêu
        self.skip_frames = 0  # Bộ đếm bỏ qua khung hình
        self.frame_count = 0
        self.start_time = 0
        self.actual_fps = 0
        
        # Các biến kết nối Arduino
        self.arduino = None
        ...
        # Quét các cổng Arduino khi khởi động
    ...
    def connect_arduino(self):
        """Kết nối với cổng Arduino đã chọn"""
    ...
    def process_video(self):
        ...
        # Các biến cho phát hiện đuối nước
        ...
        while self.is_playing:
            try:
                ...
                if elapsed_time >= 1.0:  # Cập nhật FPS mỗi giây
                    ...
                    # Cập nhật số khung hình bỏ qua dựa trên FPS thực tế so với mục tiêu
                    if self.actual_fps < self.target_fps * 0.8:  # Nếu dưới 80% FPS mục tiêu
                        self.skip_frames = min(self.skip_frames + 1, 3)  # Tăng bỏ qua, tối đa 3
                    elif self.actual_fps > self.target_fps * 1.2:  # Nếu trên 120% FPS mục tiêu
                        self.skip_frames = max(self.skip_frames - 1, 0)  # Giảm bỏ qua, tối thiểu 0
                ...
                if not ret:
                    # Kết thúc video
                    self.is_playing = False
                    self.play_btn.config(text="Phát Video")
                ...
                # Bỏ qua khung hình nếu cần thiết để cải thiện hiệu suất
                ...
                # Thêm thông tin FPS vào khung hình
                ...
                # Thêm chỉ báo thời gian đuối nước nếu phát hiện đuối nước
                if drowning_start_time is not None:
                ...
                time.sleep(0.1)  # Thêm độ trễ nhỏ khi có lỗi
            
            # Độ trễ nhỏ để không quá tải hệ thống
            time.sleep(0.01)
